...
# ══════════════════════════════════════════════════════════════════════
# COMMISSION CLAIM TRACKING SYSTEM
# ══════════════════════════════════════════════════════════════════════
...
def api_get_commission_claims():
    if 'user_id' not in session or session.get('user_type') != 'Admin':
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        res = sb_admin.table('seller_commission_claims') \
            .select('*') \
            .order('created_at', desc=True) \
            .execute()
        return jsonify({'success': True, 'claims': res.data or []})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/admin/commission_claims', methods=['POST'])
def api_create_commission_claim():
    if 'user_id' not in session or session.get('user_type') != 'Admin':
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        payload = {
            ...
            'reference_no':   data.get('reference_no', '').strip() or None,
            'notes':          data.get('notes', '').strip() or None,
        }
        res = sb_admin.table('seller_commission_claims').insert(payload).execute()
        return jsonify({'success': True, 'claim': res.data[0] if res.data else {}})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/admin/commission_claims/<int:claim_id>', methods=['PUT'])
def api_update_commission_claim(claim_id):
    if 'user_id' not in session or session.get('user_type') != 'Admin':
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        data = request.get_json()
        payload = {}
        for field in ['seller_email', 'seller_name', 'amount', 'status',
                      'claim_date', 'paid_date', 'payment_method', 'reference_no', 'notes']:
            if field in data:
                val = data[field]
...
@app.route('/api/admin/commission_claims/<int:claim_id>', methods=['DELETE'])
def api_delete_commission_claim(claim_id):
    if 'user_id' not in session or session.get('user_type') != 'Admin':
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        sb_admin.table('seller_commission_claims').delete().eq('id', claim_id).execute()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/admin/commission_claims/sellers')
def api_commission_sellers():
    if 'user_id' not in session or session.get('user_type') != 'Admin':
        return jsonify({'error': 'Unauthorized'}), 401
    try:
        from collections import defaultdict
        orders_res = sb_admin.table('orders').select(
            'seller_email, total_price, shipping_fee, status'
        ).execute()
        users_res = sb_admin.table('users').select(
            'email, first_name, last_name, business_name, role'
        ).execute()
        done = {'delivered', 'completed', 'received'}
        seller_totals = defaultdict(float)
        for o in (orders_res.data or []):
            if (o.get('status') or '').lower() in done and o.get('seller_email'):
                total = float(o.get('total_price') or 0)
                seller_totals[o['seller_email']] += round(total * 0.05, 2)
        name_map = {}
        for u in (users_res.data or []):
            if (u.get('role') or '').lower() == 'seller':
                fn  = (u.get('first_name') or '').strip()
                ln  = (u.get('last_name') or '').strip()
                biz = (u.get('business_name') or '').strip()
                name_map[u['email']] = biz or f'{fn} {ln}'.strip() or u['email']
        sellers = [
            {'email': email, 'name': name_map.get(email, email),
             'total_commission': round(total, 2)}
            for email, total in sorted(seller_totals.items(), key=lambda x: -x[1])
        ]
